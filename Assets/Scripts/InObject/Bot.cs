...
// Shared flight, sampled avoidance and moving-home docking. Jobs live in derived bots.
public abstract class Bot : MonoBehaviour
{
    ...
    [Header("Bot navigation and search")]
    ...
    protected Rigidbody homeRigidbody;
    ...
    {
        if (home == null) home = transform.parent;
        if (home != null) homeOffset = home.InverseTransformPoint(transform.position);
        ...
        InitializeTrail();
        SetNavigationState(transform.parent == home ? NavigationState.Idle : NavigationState.ReturningHome);
    }

    protected bool TickNavigation()
    {
        coolDownTimer += Time.fixedDeltaTime;
        if (home == null) { Destroy(gameObject); return false; }
        homeRigidbody = home.GetComponentInParent<Rigidbody>();
        return rb != null && PlayManager.instance != null && PlayManager.instance.playMode;
    }
    ...
    protected void NavigateToTarget(Transform target)
    {
        ...
        {
            LeaveHome();
            if (transform.parent == home) return;
        }
        else
        {
        ...
        float rangeScale)
    {
        ...
        for (int i = 0; i < avoidanceRays; i++)
        {
            ...
            {
                if (hit.transform.IsChildOf(transform) || (navigateTarget != null && hit.transform.IsChildOf(navigateTarget))) continue;
                // 过滤地面
