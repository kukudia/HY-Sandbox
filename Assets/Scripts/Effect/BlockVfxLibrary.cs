...
public sealed class BlockVfxLibrary : ScriptableObject
{
    ...
    [SerializeField] private AssetParticleEffect[] _bursts = new AssetParticleEffect[7];
    [SerializeField] private AssetParticleEffect _detachedSmoke;
    ...
    public AssetParticleEffect DetachedSmoke => _detachedSmoke;

    public static void Play(Effect kind, Vector3 position, Quaternion rotation, float scale = 1f)
    {
        BlockVfxLibrary library = Instance;
        if (library == null || !AssetParticleEffect.CanSpawnBurst) return;
        int index = (int)kind;
        if (index >= library._bursts.Length || library._bursts[index] == null) return;
        AssetParticleEffect effect = Instantiate(library._bursts[index], position, rotation);
